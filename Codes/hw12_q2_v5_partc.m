...
%%
%Variable learning rate gradient descent with weight decay
...
%     Ein(iters) = 0;
    ...
    Ein(iters) = (Ein(iters)/(4*numTrainPoints)) + (lambda/numTrainPoints)*(sum(sum(NeuralNetwork_layerData_1_W)) + sum(NeuralNetwork_layerData_2_W));
    NeuralNetwork_layerData_1_gradient = (NeuralNetwork_layerData_1_gradient/numTrainPoints) + (2*lambda/numTrainPoints)*NeuralNetwork_layerData_1_W;
    NeuralNetwork_layerData_2_gradient = (NeuralNetwork_layerData_2_gradient/numTrainPoints) + (2*lambda/numTrainPoints)*NeuralNetwork_layerData_2_W;
...
%>>>>Is the following reqd?
    ...
    for i = 1:numTestPoints
        %Forward prop
        NeuralNetwork_layerData_1_S = ((NeuralNetwork_layerData_1_Wprev)')*([1;dataTrainX(i,:)']);
        NeuralNetwork_layerData_1_X = NeuralNetwork_layerData_1_S;
        NeuralNetwork_layerData_2_S = ((NeuralNetwork_layerData_2_Wprev)')*([1;NeuralNetwork_layerData_1_X]);
        NeuralNetwork_layerData_2_X = NeuralNetwork_layerData_2_S;
        error = (NeuralNetwork_layerData_2_X - dataTrainY(i));
        error = error.*error;
        
        Etest(iters) = Etest(iters) + error;
    end
    Etest(iters) = Etest(iters)/numTestPoints;
    t1 = toc;
    fprintf('Iter: %d Ein: %f took %f sec lr: %f\n', iters, Ein(iters), t1, learningRate);
    if(iters == stopCriteriaIters || ((iters > 1) && (Etest(iters) > Etest(iters - 1))))
...
loglog(x,Etest);
hold off;
xlabel('logscale iterations');
ylabel('logscale Error');
legend('Ein','Etest');
title('num of iterations vs Error');
%%
