...
while(1)
    ...
    if(SGD)
        ...
        L1_grad = (L1_grad/numTrainSamples) + (2*lambda*L1_W/numTrainSamples);
        L2_grad = (L2_grad/numTrainSamples) + (2*lambda*L2_W/numTrainSamples);
        error = (error/(numTrainSamples)) + (lambda/numTrainPoints)*(sum(sum(L1_W)) + sum(L2_W));
    end
