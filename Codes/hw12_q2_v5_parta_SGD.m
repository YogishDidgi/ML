...
% NeuralNetwork(1).layerData(1).W = randi(5)*rand(size(NeuralNetwork(1).layerData(1).W,1),1)*(rand(size(NeuralNetwork(1).layerData(1).W,2),1))';
% NeuralNetwork(1).layerData(2).W = randi(5)*rand(size(NeuralNetwork(1).layerData(2).W,1),1)*(rand(size(NeuralNetwork(1).layerData(2).W,2),1))';
NeuralNetwork(1).layerData(1).W = randi(5)*rand(size(NeuralNetwork(1).layerData(1).W));
NeuralNetwork(1).layerData(2).W = randi(5)*rand(size(NeuralNetwork(1).layerData(2).W));
%%
...
%     for i = 1:numTrainPoints
        %Forward prop
        NeuralNetwork_layerData_1_S = ((NeuralNetwork_layerData_1_W)')*([1;dataTrainX(randIndex,:)']);
        ...
        error = (NeuralNetwork_layerData_2_X - dataTrainY(randIndex));
        error = error.*error;
        
        Ein(iters) = Ein(iters) + error;
        %Backward prop
        NeuralNetwork_layerData_2_delta = 2*(NeuralNetwork_layerData_2_X - dataTrainY(randIndex))*(1);
        ...
        %Gradient calc
        NeuralNetwork_layerData_1_gradient = NeuralNetwork_layerData_1_gradient + ([1; dataTrainX(randIndex,:)'])*(NeuralNetwork_layerData_1_delta');
...
%     end
    ...
    if((iters == stopCriteriaIters) || (learningRate == 0))
