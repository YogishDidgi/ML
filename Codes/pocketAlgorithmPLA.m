function [finalW, error] = pocketAlgorithmPLA(X_Augmented, W, Y, maxIters)
[numSamples, dim] = size(X_Augmented);
prevW = W;
newW = W;
iter = 0;
errorPrevious = evaluateError(X_Augmented, W, Y);
finalW = W;
errorCurrent = errorPrevious;
errorInitial = errorPrevious;

while(iter < maxIters)
    flag = false;
%     disp(newW);
%     disp(iter);
    for i=1:numSamples
        if(Y(i) == checkSample(X_Augmented(i,:), newW))
            continue;
        else
            flag = true;
            iter = iter + 1;
            newW = prevW + Y(i).*X_Augmented(i,:)';
            prevW = newW;
        end
        if(flag == true)
            break;
        end
    end
    errorCurrent = evaluateError(X_Augmented, newW, Y);
    if(errorCurrent < errorPrevious)
        errorPrevious = errorCurrent;
        finalW = newW;
    end
    if(flag == false)
        break;
    end
end
error = errorPrevious;
% errorInitial
% error

function output = checkSample(X, W)
output = sign((X*W));